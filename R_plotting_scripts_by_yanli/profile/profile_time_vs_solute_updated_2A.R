# Name: Yanli Xu
# Date: Jan 6th, 2017
# Note: This script is used for plotting time vs solute in "mean_profile" files
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("profile_time_vs_solute_body_updated_2A.R"), the script will start working 
...
# > source("profile_time_vs_solute_updated_2A.R")
...
time_vs_solute <- function()
{
  ...
  for(solute_num in 2:length(solute_names))
  {
    ...
    {
      exp <- eval(parse(text = exp_names[i]))
      title = paste("Profile_Time_vs", solute_names[solute_num], conc[i], sep = "_")
