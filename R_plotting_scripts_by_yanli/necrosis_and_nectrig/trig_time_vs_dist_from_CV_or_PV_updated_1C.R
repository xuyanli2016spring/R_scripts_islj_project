...
# make the terminal work under R, then enter command source("trig_time_vs_dist_from_CV_or_PV_updated_1C.R"), the script will start working
# Input file: XXXX_nectrig.csv
# Output: four pages of plots, every plot is for one concentrtion. The difference between this script and "trig_time_vs_dist_from_CV_or_PV_updated_1B.R" is that this script doesn't have grey dots underlying the lines

# The following is an example of running this script
# > source("trig_time_vs_dist_from_CV_or_PV_updated_1B.R")
...
time_vs_nec_dist <- function()
{
  ...
  for(z in zone)
  {
    for(t in type_dist)
    {
      ...
      {
        exp <- eval(parse(text = exp_names[i]))
        title <- paste("Trig_Time_vs_", t, "_dist_to_", z, "_", conc[i], sep = " ")
        ...
        plot(mav(exp[, c(paste(t, ".Dist_from_", z, sep=""))]) ~ exp$Time_of_NecTrig, type = "l", main = title, col = color[i], xlab = "Time", ylab = paste("Dist_to_", z, sep = ""), xlim = c(0, run_time), ylim = c(y_min, y_max))
      }
    }
  }
}
