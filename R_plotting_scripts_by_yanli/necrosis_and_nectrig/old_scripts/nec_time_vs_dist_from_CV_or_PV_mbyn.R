...
# make the terminal work under R, then enter command source("nec_time_vs_dist_from_CV_or_PV_mbyn.R"), the script will start working
# Input file: XXXX_necrotic.csv

# The following is an example of running this script
# > source("nec_time_vs_dist_from_CV_or_PV_mbyn.R")
...
time_vs_nec_dist <- function()
{
  ...
  for(i in 1: length(conc))
  {
    ...
    title <- paste("Nec_Time_vs_", type_dist, "_dist_to_", zone, "_", conc[i], sep = " " )
