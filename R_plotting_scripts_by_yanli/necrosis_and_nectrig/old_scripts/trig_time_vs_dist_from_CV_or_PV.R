...
# make the terminal work under R, then enter command source("trig_time_vs_dist_from_CV_or_PV.R"), the script will start working 
# Input file: XXXX_nectrig.csv

# The following is an example of running this script
# > source("trig_time_vs_dist_from_CV_or_PV.R")
...
# You already got the plot, please input x and y coordinates to put legends: 25000, 15
...
#   y_limits <- as.numeric(unlist(strsplit(y_lim_str, "[, ]+")))
  ...
  # Ask user to choose median or mean dist
  ...
  plot(c(), c(), xlim = c(0, run_time), ylim = y_limits, main = paste("Nectrig time vs ", type_dist, ".dist_from_", zone), xlab = "Time", ylab = paste("Distance from ", zone, sep = ""))
