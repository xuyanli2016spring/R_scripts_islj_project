# Name: Yanli Xu
# Date: Nov 29th, 2016
# Note: This script is used for plotting time vs necrosis for multiple files, the plots can be displayed in 1 by 1, or m by n
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("time_vs_necrotic.R"), the script will start working 
# Input file: XXXX_necrotic.csv

# The following is an example to run this script
# > source("time_vs_necrotic.R")
...
# You have  9  plots, please input how many rows and columns do you want to display the plots: 1, 1
...
# Based on your input, the bottom and upper limits of x-axis are 0 and 36000, Please input bottom and upper limits of y-axis to represent the range of necrosis: 0, 15000
...
time_vs_nec <- function()
{
  ...
  y_lim_str <- readline(paste("Based on your input, the bottom and upper limits of x-axis are 0 and ", run_time, ", Please input bottom and upper limits of y-axis to represent the range of necrosis: ", sep = ""))
  y_limits <- as.numeric(unlist(strsplit(y_lim_str, "[, ]+")))
  
  if(identical(mf, c(1, 1)))
  {
    plot(c(), c(), type = "l", main = "time vs necrosis for different concentrations", xlab = "time", ylab = "necrosis", xlim = c(0, run_time), ylim = y_limits)
  ...
  else
  {
    ...
    {
      exp <- eval(parse(text = exp_names[i]))
      title = paste(exp_names[i], "time vs necrosis", sep = " ")
      plot(exp$Cumu_Necrotic ~ exp$Time_of_Necrosis, type = "l", main = title, xlim = c(0, run_time), ylim = y_limits, col = color[i], xlab = "Time", ylab = "Necrosis")
    }
  }
}
time_vs_nec()
