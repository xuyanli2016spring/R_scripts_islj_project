# Name: Yanli Xu
# Date: Nov 30th, 2016
# Note: This script is used for plotting concentration vs nectrig at different time for multiple files
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("concentration_vs_nectrig.R"), the script will start working
# Input file: XXXX_nectrig.csv

# The following is an example of running this script
# > source("concentration_vs_nectrig.R")
...
# Please input the bottom and upper limits to represent the range of nectrig: 0, 15000
# Please input the cycleLimit of these experiments: 36000
# Please input 10 different colors to represent different hours, you may choose from these colors, black, blue, green, red, burlywood4, forestgreen, firebrick, blue4, purple, chocolate, cyan, gold, darkviolet, brown: black, blue, green, red, burlywood4, forestgreen, firebrick, blue4, purple
# You already got the plot, please input x and y coordinates to put legends: 0, 15000

nectrig_vs_conc <- function()
{
  ...
  y_lim_str <- readline("Please input the bottom and upper limits to represent the range of nectrig: ")
  y_limits <- as.numeric(unlist(strsplit(y_lim_str, "[, ]+")))
  
  plot(c(), c(), xlim = x_limits, ylim = y_limits, xlab = "Concentration", ylab = "Necrosis", main = "concentration vs nectrig at different time")
  ...
  for(hrs in 1:(run_time/3600))
  {
    ...
    for(i in 1:length(conc))
    {
      ...
      exp_sub <- subset(exp[exp$Time_of_NecTrig <= (3600 * hrs), ])
      final_encrosis[i] <-  max(exp_sub$Cumu_Nectrig)  
    }
