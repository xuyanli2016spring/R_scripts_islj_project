...
# make the terminal work under R, then enter command source("trig_time_vs_dist_from_CV_or_PV_updated_1B.R"), the script will start working
# Input file: XXXX_nectrig.csv
# Output: four pages of plots, every plot is for one concentrtion, and has one line imposed on the grey dots 
  ...
  for(z in zone)
  {
    for(t in type_dist)
    {
      ...
      for(i in 1:length(conc))
      {
        ...
        plot(exp[, c(paste(t, ".Dist_from_", z, sep=""))] ~ exp$Time_of_NecTrig, main = title, col = "grey85", xlab = "Time", ylab = paste("Dist_to_", z, sep = ""), xlim = c(0, run_time), ylim = c(y_min, y_max))
