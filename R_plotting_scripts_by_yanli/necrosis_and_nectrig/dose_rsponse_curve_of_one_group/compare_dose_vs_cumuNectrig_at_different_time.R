# Name: Yanli Xu
# Date: Feb 13th, 2017
# To compare dose vs cumu_nectrig at multiple times 
...
nectrig_vs_dose <- function()
{
  ...
  conc_str <- readline(paste("Enter ", length(files),  " different doseRatios for the experiments: ", sep = ""))
...
#   num_mc <- as.numeric(unlist(strsplit(mc_str, "[, ]+")))
  ...
  y_lim_str <- readline("Please input the bottom and upper limits to represent the range of cumu_nectrig, for expample (0, 15000): ")
  y_limits <- as.numeric(unlist(strsplit(y_lim_str, "[, ]+")))
  
  plot(c(), c(), xlim = x_limits, ylim = y_limits, xlab = "Doses", ylab = "Cumu_Nectrig", main = "Doses vs Cumu_Nectrig")
  ...
  for(hrs in 1:(run_time/1800))
  {
    ...
    lines(cumu_nectrig ~ conc, col = color[hrs])
  }
