...
# make the terminal work under R, then enter command source("time_vs_solute_medium_update_1B.R"), the script will start working 
...
# > source("time_vs_solute_medium_update_1B.R")
...
time_vs_solute <- function()
{
  ...
  for(solute_num in 2:length(solute_names))
  {
    ...
    {
      exp <- eval(parse(text = exp_names[i]))
      title = paste("medium_Time_vs", solute_names[solute_num], conc[i], sep = "_")
