...
# make the terminal work under R, then enter command source("time_vs_solute_medium_update.R"), the script will start working 
...
# Please input the working directory: /Users/yanlixu/Desktop/experiment_data/161011_161020
...
time_vs_solute <- function()
{
  ...
  {
    exp <- eval(parse(text = exp_names[i]))
    title = paste("Medium_Time_vs", solute_names[solute_num + 1], conc[i], sep = "_")
