...
# > source("compare_rxn_of_three_zones_1A.R")
...
# Please input 3 different colors to represent 3 different zones, you may choose from these colors, black, blue, green, red, burlywood4, forestgreen, firebrick, blue4, purple, chocolate, cyan, gold, darkviolet, brown: black, red, green
...
#   rxn_num <- as.numeric(unlist(strsplit(rxn_str, "[, ]+")))
  ...
  for(rxn_num in 1:length(rxn_names))
  {
    ...
    for(i in 1:length(conc))
    {
      ...
      title = paste("Rxn_Time_vs", rxn_names[rxn_num], conc[i], sep = "_")
