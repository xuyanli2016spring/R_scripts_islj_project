# Name: Yanli Xu
# Date: Nov 29th, 2016
# Note: This script is used for plotting time vs perZone necrotic dist2cv for all zones, the output is m*n plots, every plot for one concentration
...
# make the terminal work under R, then enter command source("time_vs_necrotic_dist2cv_all_zones.R"), the script will start working
# input file: XXXX-XX-XX-XXXX-XXXX_perZone_necrotic.csv

# The following is an example of running this script
# > source("time_vs_necrotic_dist2cv_all_zones.R")
...
# Please input bottom and upper limits of y-axis to represent the range of per_zone_necrotic dist_to_CV, for example 0, 50: 0, 50
...
time_vs_dist2cv <- function()
{
  ...
  y_lim_str <- readline(paste("Please input bottom and upper limits of y-axis to represent the range of per_zone_necrotic dist_to_CV, for example 0, 50: ", sep = ""))
    ...
    for(zone in 1:4)
    {
      ...
      for(index_1 in 1: (nrow(exp)-99))
      {
        ...
        if((nrow(exp)-99) <= 99)
        {
          ...
          MA_Time_of_Necrotic <- rep(0, nrow(exp))
        }
        else
        {
          count = 1
          ...
          MA_Time_of_Necrotic[index_1] = mean(sub_time)
        }
      }

      lines(MA_Dist_from_CV ~ MA_Time_of_Necrotic, col = color[zone])
    }
    if(i == 1)
    {
