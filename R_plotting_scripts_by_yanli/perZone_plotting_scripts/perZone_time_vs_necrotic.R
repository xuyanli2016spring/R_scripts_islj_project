# Name: Yanli Xu
# Date: Nov 29th, 2016
# Note: This script is used for plotting time vs zone_1_necrosis, time vs zone_2_necrosis, time vs zone_3_necrosis, time vs zone_nearPV_necrosis, for multiple files
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("perZone_time_vs_necrotic.R"), the script will start working 
# input file: XXXX-XX-XX-XXXX-XXXX_perZone_necrotic.csv

# The following is an example of running this script
# > source("perZone_time_vs_necrotic.R")
...
# Please input bottom and upper limits of y-axis to represent the range of Z3 necrotic value: 0, 5000
# You already got the plot, please input x and y coordinates to put legends: 0, 5000
...
perZone_time_vs_nec <- function()
{
  ...
  y_lim_str <- readline(paste("Please input bottom and upper limits of y-axis to represent the range of ", zone, " necrotic value: ", sep = ""))
  y_limits <- as.numeric(unlist(strsplit(y_lim_str, "[, ]+")))
  
  title <- paste("Cumu_Necrotic_", zone, " for different concentrations", sep = "")
  plot(c(), c(), main = title, xlab = "Time", ylab = "necrotic", ylim = y_limits, xlim = c(0, run_time))
  for (i in 1:length(conc))
  {
    exp <- eval(parse(text = exp_names[i]))
    y_value <- exp[, c(paste("Cumu_Necrotic_", zone, sep = ""))]
