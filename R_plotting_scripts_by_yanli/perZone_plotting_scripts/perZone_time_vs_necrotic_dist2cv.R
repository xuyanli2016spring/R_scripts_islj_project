# Name: Yanli Xu
# Date: Nov 29th, 2016
# Note: This script is used for plotting time vs necrotic_dist2cv for zone_1, zone2, zone3 and zone_nearPV.
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("perZone_time_vs_necrotic_dist2cv.R"), the script will start working 
# input file: XXXX-XX-XX-XXXX-XXXX_perZone_necrotic.csv

# The following is an example of running this script
# > source("perZone_time_vs_necrotic_dist2cv.R")
...
# You already got the plot, please input x and y coordinates to put legends: 30000, 6
...
perZone_time_vs_dist <- function()
{
  ...
  title <- paste("Necrotic_DistCV_", zone, " for different concentrations", sep = "")
  ...
  for(i in 1: length(conc))
  {
    ...
    else
    {
      ...
      lines(MA_Dist_from_CV ~ MA_Time_of_Necrotic, col = color[i])
    }
  }
