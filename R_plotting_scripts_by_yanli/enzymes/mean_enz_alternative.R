...
#   y_limit_2 <- exp[1, 4]
  ...
  legend_str <- readline("please input bottom and upper value of y-axis to represent the enzyme level, for example 1000, 2000: ")
  y_limits <- as.numeric(unlist(strsplit(legend_str, "[, ]+")))
  ...
  for (i in 1:length(conc))
  {
    ...
    plot(exp$Zone.3 ~ exp$Time, type = "l", xlab = "Time", ylab = "Enzyme", main = paste("mean_enz", enz_file, conc[i], sep="_"), ylim = y_limits, xlim = c(0, run_time))
