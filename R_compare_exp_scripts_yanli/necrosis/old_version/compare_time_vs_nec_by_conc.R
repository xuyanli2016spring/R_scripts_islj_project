# Name: Yanli Xu
# Date: Dec 16th, 2016
# Note: This script is used for compare the necrosis_vs_time of the same concentrations among different groups, the output is one plot for every concentration. 
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("compare_time_vs_nec_by_conc.R"), the script will start working 
...
# > source("compare_time_vs_nec_by_conc.R")
...
# Please input a name to distainguish this group, for example, infusion_liver: inf_cul
...
compare_time_vs_nec <- function()
{
  ...
  for(exp_num in 1:num_of_exp_perGroup)
  {
    ...
    # plot the files
    plot(c(), c(), type = "l", main = paste("time vs necrosis of ", conc[exp_num], sep = ""), xlab = "time", ylab = "necrosis", xlim = c(0, run_time), ylim = y_limits)
