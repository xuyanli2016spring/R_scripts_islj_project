# Name: Yanli Xu
# Date: Jan 17th, 2017
# Note: This script is used for compare the necrotic_vs_time between groups, the output is one plot for every group. 
# How to use this script: open terminal, cd to the directory of this script, enter command R will
# make the terminal work under R, then enter command source("compare_time_vs_nec_by_group_updated.R"), the script will start working 
...
# > source("compare_time_vs_nec_by_group_updated.R")
...
# You already got the plot of the first group, please input x and y coordinates to put legends: 10000, 20000


compare_time_vs_necrotic <- function()
{
  #########
  ...
  for(group_num in 1:num_of_groups)
  {
    ...
    plot(c(), c(), type = "l", main = paste("time vs necrotic of ", exp_group_names[group_num], sep = ""), xlab = "time", ylab = "necrotic", xlim = c(0, run_time), ylim = c(0, y_max))
