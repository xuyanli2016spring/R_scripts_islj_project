...
# make the terminal work under R, then enter command source("time_vs_solute_body_update_1A.R"), the script will start working 
# Input file: XXXX-XX-XX-XXXX-XXXX_mean_body_.csv
# plot all the time vs body_solute ("S", "nMD", "MitoDD", "oAPAP", "G", "Marker", "Repair", "N" )
...
time_vs_solute <- function()
{
  ...
  # Read the files
  #files <- list.files(path = work_dir, pattern = "mean_medium|mean_body", recursive = T)
  files <- list.files(path = work_dir, pattern = "medium|body", recursive = T)
  # Ask user to input the concentrations
  conc_str <- readline(paste("Enter ", length(files),  " different experiment names to represent different experiments: ", sep = ""))
...
#   solute_num_legend <- as.numeric(unlist(strsplit(solute_str, "[, ]+"))) + 1
  ...
  for(solute_num in 2:length(solute_names))
  {
    ...
    plot(c(), c(), type = "l", main = paste("Time_vs", solute_names[solute_num], "of_body/medium", sep = "_"), xlab = "time", ylab = solute_names[solute_num], xlim = c(0, run_time), ylim = c(y_min, y_max))
    for(i in 1:length(conc))
     {
      exp <- eval(parse(text = exp_names[i]))
